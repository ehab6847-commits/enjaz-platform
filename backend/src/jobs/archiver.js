...
/**
 * Archives NEW/VIEWED requests older than 24 hours.
 * Runs every hour.
 */
...
// ─── Delete Old Archives ───────────────────────────────────────────────────────
/**
 * Deletes ARCHIVED requests older than 7 days to keep the database clean.
...
const deleteOldArchives = async () => {
  try {
    ...
    if (count > 0) {
      logger.info(`[Archiver] Deleted ${count} old archived request(s)`);
    }
  } catch (err) {
 ...
 */
const initCronJobs = () => {
  // Archive requests older than 24h — every hour
  cron.schedule('0 * * * *', archiveOldRequests, {
    name: 'archive-old-requests',
    timezone: 'Asia/Riyadh',
  });

  // Delete archives older than 7 days — daily at 2:00 AM
  cron.schedule('0 2 * * *', deleteOldArchives, {
  ...
  logger.info('Cron jobs scheduled:', [
    'archive-old-requests (every hour)',
    'delete-old-archives (daily 2:00 AM)',
